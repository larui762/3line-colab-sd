...
function sed_for {
    ...
    if [[ $option == "installation" ]]; then
        ...
        sed -i -e '''/        prepare_environment()/a\        os.system\(f\"""sed -i -e ''\"s/dict()))/dict())).cuda()/g\"''  /content/drive/Mydrive/SD/repositories/stable-diffusion-stability-ai/ldm/util.py""")''' $base_dir/launch.py
